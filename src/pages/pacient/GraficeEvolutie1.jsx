...
function GraficeEvolutie1({ userId }) {
  console.log('userId primit:', userId); // <-- aici
  const [data, setData] = useState([]);

useEffect(() => {
  if (!userId) return;
  async function fetchData() {
    const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/pacient/datefiziologice?userId=${userId}`);
...
export default GraficeEvolutie1;